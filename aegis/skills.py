...
| precommit-config-protect | planted a `repo: local` hook with an `entry:` command in .pre-commit-config.yaml (runs with no upstream repo/pinned rev on the next `pre-commit run` / `git commit`, by anyone) | ask the human; they can set AEGIS_ALLOW_PRECOMMIT_CONFIG=1 after review |
...
2. Read the policy YAML files it names (they are small) and summarize:
   ...
   `git_hooks`, `agent_def`, `shell_persist`, `direnv`, `package_manifest`, `service_persist`, `ld_preload`, `devcontainer_exec`, `vscode_tasks_exec`, `path_hijack`, `claude_hooks`, `conftest`, `pysite`, `ipython_startup`, `precommit_config`, `inject`, `failures`, `completion`,
...
2. Read the existing YAML; make the smallest change that expresses the intent:
   ...
   - knobs: `default_action`, `egress`, `workspace`, `install_review`,
     ...
     `claude_hooks`, `conftest`, `pysite`, `ipython_startup`, `precommit_config`,
