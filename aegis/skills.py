...
| pnpmfile-exec-protect | wrote pnpm's own hook file (.pnpmfile.cjs/.js) or redirected pnpm's `pnpmfile` config key (runs as arbitrary Node.js, before any dependency's own lifecycle scripts, on the next `pnpm install`/`add`/`update`/`import`) | ask the human; they can set AEGIS_ALLOW_PNPMFILE_EXEC=1 after review |
