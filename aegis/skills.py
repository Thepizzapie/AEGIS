...
| helm-hooks-protect | planted a `helm.sh/hook` lifecycle annotation on a resource under a chart's templates/ (Helm itself runs it on the next `helm install`/`upgrade`/`uninstall`, often via an unattended GitOps controller — no confirmation gate at all) | ask the human; they can set AEGIS_ALLOW_HELM_HOOKS=1 after review |
...
2. Read the existing YAML; make the smallest change that expresses the intent:
   ...
   - knobs: `default_action`, `egress`, `workspace`, `install_review`,
     ...
     `claude_hooks`, `statusline`, `permission_bypass`, `conftest`, `pysite`, `ipython_startup`, `cloud_cred_exec`, `terraform_exec`, `helm_hooks`, `fetch_to_file`,
