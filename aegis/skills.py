...
| claude-statusline-protect | planted a `statusLine` command entry in .claude/settings.local.json (runs as Claude Code's own subprocess, event-driven throughout the session, outside the tool-call loop Aegis evaluates) | ask the human; they can set AEGIS_ALLOW_CLAUDE_STATUSLINE=1 after review |
...
2. Read the policy YAML files it names (they are small) and summarize:
   ...
   `git_hooks`, `agent_def`, `shell_persist`, `direnv`, `package_manifest`, `git_config_exec`, `git_attributes_exec`, `gitmodules`, `service_persist`, `ld_preload`, `devcontainer_exec`, `vscode_tasks_exec`, `path_hijack`, `claude_hooks`, `claude_statusline`, `conftest`, `pysite`, `ipython_startup`, `fetch_to_file`, `inject`, `failures`, `completion`,
...
2. Read the existing YAML; make the smallest change that expresses the intent:
   ...
   - knobs: `default_action`, `egress`, `workspace`, `install_review`,
     ...
     `claude_hooks`, `claude_statusline`, `conftest`, `pysite`, `ipython_startup`, `fetch_to_file`,
