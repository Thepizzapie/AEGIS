...
| secret-material-exfil | a literal secret (private key, cloud/vendor token, webhook URL) is in a tool call reaching the network | ask the human; they can set AEGIS_ALLOW_SECRET_MATERIAL=1 or allowlist the tool after review |
...
2. Read the policy YAML files it names (they are small) and summarize:
   ...
   opt-in knobs are on (`install_review`, `mcp_config`, `secret_exfil`,
   `inject`, `failures`, `completion`, `team`, `compaction`, `permission`,
   `mcp`).
...
2. Read the existing YAML; make the smallest change that expresses the intent:
   ...
   - knobs: `default_action`, `egress`, `workspace`, `install_review`,
     `mcp_config`, `secret_exfil`, `inject`, `failures`, `completion`, `team`,
