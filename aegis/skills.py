...
| prompt-secret-leak | submitted prompt contained a live-looking secret | remove/rotate the credential and resend; never re-paste it to argue the point |\
"""
...
2. Read the policy YAML files it names (they are small) and summarize:
   ...
   `completion`, `secrets`, `team`, `compaction`, `permission`, `mcp`).
...
2. Read the existing YAML; make the smallest change that expresses the intent:
   ...
   - knobs: `default_action`, `egress`, `workspace`, `install_review`,
     `mcp_config`, `inject`, `failures`, `completion`, `secrets`, `team`,
