...
| ld-preload-protect | wrote /etc/ld.so.preload (dlopen()'d into every dynamically-linked program run on the machine from now on) or /etc/ld.so.conf(.d) (shared-library search-path hijack) | ask the human; they can set AEGIS_ALLOW_LD_PRELOAD=1 after review |
| sudoers-pam-protect | wrote /etc/sudoers or /etc/sudoers.d/* (directly or via `visudo`), or a PAM module in /etc/pam.d/*/etc/pam.conf (grants root privilege or hijacks authentication on the very next sudo/login/auth attempt, no reboot needed) | ask the human; they can set AEGIS_ALLOW_SUDOERS_PAM=1 after review |
...
2. Read the policy YAML files it names (they are small) and summarize:
   ...
   `git_hooks`, `hook_manager`, `agent_def`, `skills_protect`, `shell_persist`, `direnv`, `package_manifest`, `pnpmfile_exec`, `git_config_exec`, `git_attributes_exec`, `gitmodules`, `service_persist`, `ld_preload`, `sudoers_pam`, `devcontainer_exec`, `vscode_tasks_exec`, `path_hijack`, `claude_hooks`, `statusline`, `permission_bypass`, `conftest`, `pysite`, `ipython_startup`, `cloud_cred_exec`, `terraform_exec`, `fetch_to_file`, `inject`, `failures`, `completion`,
...
2. Read the existing YAML; make the smallest change that expresses the intent:
   ...
   - knobs: `default_action`, `egress`, `workspace`, `install_review`,
     ...
     `service_persist`, `ld_preload`, `sudoers_pam`, `devcontainer_exec`, `vscode_tasks_exec`, `path_hijack`,
