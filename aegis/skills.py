...
| interp-env-protect | set BASH_ENV/PYTHONSTARTUP, or NODE_OPTIONS/PERL5OPT/RUBYOPT with a module-load flag (makes the interpreter auto-run attacker code on its very next invocation, no file write or restart needed) | ask the human; they can set AEGIS_ALLOW_INTERP_ENV=1 after review |
...
2. Read the existing YAML; make the smallest change that expresses the intent:
   ...
   - knobs: `default_action`, `egress`, `workspace`, `install_review`,
     ...
     `claude_hooks`, `statusline`, `permission_bypass`, `conftest`, `pysite`, `ipython_startup`, `interp_env_exec`, `cloud_cred_exec`, `terraform_exec`, `fetch_to_file`,
