...
| dir-locals-protect | planted an `(eval . FORM)` binding (or its own confirmation-silencing switch) in .dir-locals.el/.dir-locals-2.el (Emacs `eval`s it automatically, unattended, the next time anyone opens any file under this directory tree in Emacs, no Run/build/git/CI trigger needed) | ask the human; they can set AEGIS_ALLOW_DIR_LOCALS_EXEC=1 after review |
...
2. Read the existing YAML; make the smallest change that expresses the intent:
   ...
   - knobs: `default_action`, `egress`, `workspace`, `install_review`,
     ...
     `service_persist`, `ld_preload`, `devcontainer_exec`, `vscode_tasks_exec`, `jetbrains_watcher_exec`, `dir_locals_exec`, `path_hijack`,
