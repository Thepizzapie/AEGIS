...
| gcp-adc-exec-protect | planted a GCP Application Default Credentials executable-sourced-credential block (`credential_source.executable.command`, any path, or the `gcloud ... create-cred-config --executable-command` CLI form) — runs on every future GCP token refresh through that credential file | ask the human; they can set AEGIS_ALLOW_GCP_ADC_EXEC=1 after review |
...
2. Read the policy YAML files it names (they are small) and summarize:
   ...
   `git_hooks`, `agent_def`, `skills_protect`, `shell_persist`, `direnv`, `package_manifest`, `git_config_exec`, `git_attributes_exec`, `gitmodules`, `service_persist`, `ld_preload`, `devcontainer_exec`, `vscode_tasks_exec`, `path_hijack`, `claude_hooks`, `statusline`, `conftest`, `pysite`, `ipython_startup`, `cloud_cred_exec`, `gcp_adc_exec`, `fetch_to_file`, `inject`, `failures`, `completion`,
...
2. Read the existing YAML; make the smallest change that expresses the intent:
   ...
   - knobs: `default_action`, `egress`, `workspace`, `install_review`,
     ...
     `claude_hooks`, `statusline`, `conftest`, `pysite`, `ipython_startup`, `cloud_cred_exec`, `gcp_adc_exec`, `fetch_to_file`,
