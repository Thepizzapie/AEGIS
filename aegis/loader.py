...
def load_policy(path) -> Policy:
    ...
    return Policy(rules=st["rules"], default_action=st["default"],
                  ...
                  pnpmfile_exec=st["pnpmfile_exec"],
                  yarn_exec=st["yarn_exec"],
                  ...
                  jetbrains_watcher_exec=st["jetbrains_watcher_exec"],
                  dir_locals_exec=st["dir_locals_exec"],
                  path_hijack=st["path_hijack"],
                  claude_hooks=st["claude_hooks"],
                  statusline=st["statusline"],
                  permission_bypass=st["permission_bypass"],
                  claude_env=st["claude_env"],
                  claude_cred_helper=st["claude_cred_helper"],
...
def _merge_file(data: dict, fname: str, st: dict) -> None:
    ...
    for key in ("install_review", "mcp_config", "mcp_tool_integrity", "ci_workflow", "git_hooks", "hook_manager",
                ...
                "devcontainer_exec",
                "vscode_tasks_exec", "jetbrains_watcher_exec", "dir_locals_exec", "path_hijack", "claude_hooks", "statusline",
