...
| Hidden-Unicode prompt injection | Invisible Unicode tag-block characters, and chained zero-width/variation-selector runs (any mixture) hiding instructions in a submitted prompt (bidi-override text is monitored, not blocked, by default) | Env var only (`AEGIS_ALLOW_HIDDEN_UNICODE=1`, set before launch — not an in-band comment) |
...
**Hidden-Unicode prompt injection**, in more detail: every other guard above acts on a tool call the model already decided to make. `UserPromptSubmit` — the hook that fires before a prompt (human-typed, or machine-fed in a headless/CI run: a GitHub issue body, a PR comment, a webhook payload relayed as the turn) is processed — had no guard at all, despite being wired up and blockable. That's the entry point a steganographic prompt injection actually needs: text that renders as blank or ordinary to whoever reviews it, but still reads as instructions to the model. Aegis denies two near-zero-legitimate-use techniques — Unicode tag-block characters ("ASCII smuggling"; the small, fixed set of standardized region-flag emoji, e.g. Scotland's, is carved out by exact match, not by shape, so a real flag isn't blocked and a chunked-payload-disguised-as-flags trick isn't either), and a chained run of zero-width and/or variation-selector characters in any mixture ("emoji smuggling" and zero-width encoding, checked as one combined class so alternating between the two can't evade a per-class threshold) — and monitors (audits, doesn't block by default) bidi text-direction overrides, which have rare legitimate use. Configure via `policy.hidden_unicode: {mode: deny|monitor|off, bidi_mode: deny|monitor|off}`. The override is deliberately **not** the shell guards' in-band `# aegis-allow` comment: here the whole prompt — payload and any embedded "override" alike — can be a single artifact fully authored by whatever fed it in, so an out-of-band `AEGIS_ALLOW_HIDDEN_UNICODE=1` a human sets before launch is the only way past it.
...
- **Guards are a denylist.** They catch known-dangerous shapes, not every possible one. Known gaps: bucket-to-bucket cloud transfers and aliased clients (`mc`, `doctl`) that the cloud-CLI exfil guard doesn't parse, in-place edits (`sed -i`), `git -c` inline-config force-push; the hidden-Unicode guard's invisible-run check is a consecutive-run threshold, so capping every invisible sub-run below the threshold and inserting one incidental visible character between each can still smuggle an arbitrarily long hidden message. Deny-by-default egress is the backstop. Found a bypass? That's a bug worth reporting.
