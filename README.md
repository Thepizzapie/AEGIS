...
| Prompt secret guard | A live credential (API key/token/private key shape) pasted or smuggled into the raw prompt (`UserPromptSubmit`) | No |
...
- **Guards are a denylist.** They catch known-dangerous shapes, not every possible one. Known gaps: bucket-to-bucket cloud transfers and aliased clients (`mc`, `doctl`) that the cloud-CLI exfil guard doesn't parse, in-place edits (`sed -i`), `git -c` inline-config force-push, and a credential that's been encoded/split/base64-wrapped past what the prompt secret guard's `SECRET_RE` shapes match. Deny-by-default egress is the backstop. Found a bypass? That's a bug worth reporting.
- **Redaction touches the whole audit trail, not just prompts.** The prompt secret guard's redaction runs on every event's recorded `args`/`output`, so a false-positive `SECRET_RE` match inside legitimate tool output is masked there too — a deliberate trade (never log a live credential) that can blunt grounding evidence quoting that exact text. The `allow` regex exemption on `policy.prompt_guard` is for prompt content only; avoid writing a real secret's literal value into policy YAML to exempt it — that recreates the problem the guard exists to prevent.
