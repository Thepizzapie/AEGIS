...
| Shell-startup / SSH persistence | Writes to a shell startup/profile file (`~/.bashrc`/`~/.zshrc`/`~/.profile`/fish's `config.fish`/`/etc/profile.d/*.sh`/a PowerShell `$PROFILE`) — runs with the human's full privileges the next time they open a shell, no git/CI/session-restart trigger needed — or an SSH persistence target (`~/.ssh/authorized_keys`, `/etc/ssh/sshd_config`, `/etc/ssh/ssh_config`) | Human only |
