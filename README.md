...
| Secret exfiltration | A live-looking secret (a private-key PEM block, or a vendor-format API token — AWS/GitHub/GitLab/Slack/Stripe/Google/npm/Anthropic/OpenAI) reaching the network — via a shell command, `WebFetch`, or **any MCP tool call**. The first containment check that inspects an MCP tool's argument content at all, not just the narrow cloud-metadata URL — an ordinary-looking "post a message"/"file an issue"/"create a gist" MCP tool is just as good an exfil sink as a raw `curl` | No |
