...
| Self-protection | Deleting/editing `.aegis`, `.claude/settings.json` (and its personal/local override `.claude/settings.local.json` — untracked by convention, same `hooks`/`permissions.allow` power, so a planted change there is invisible to `git diff`/code review), or Aegis's own source; `aegis uninstall`/`pull` | No |
