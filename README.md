...
| Hidden-Unicode prompt injection | Invisible Unicode tag-block / variation-selector / chained zero-width chars hiding instructions in a submitted prompt (bidi-override text is monitored, not blocked, by default) | Env var only (`AEGIS_ALLOW_HIDDEN_UNICODE=1`, set before launch — not an in-band comment) |
