"""Claude Code credential-helper hijack protection guard — blocks planting/
altering one of five credential-helper keys (``apiKeyHelper``,
``awsAuthRefresh``, ``awsCredentialExport``, ``gcpAuthRefresh``,
``otelHeadersHelper``) in ``.claude/settings.local.json``, the project-local,
gitignored-by-default sibling of ``.claude/settings.json`` that
`rule_claude_hooks_protect`/`rule_statusline_protect`/`rule_permission_
bypass_protect`/`rule_claude_env_protect` already guard for their own keys
in this file.

Each key names a command Claude Code itself execs, on its own schedule, with
no tool-call trigger at all — ``apiKeyHelper`` alone is re-run every 5
minutes by default for the life of the session, its stdout sent as the API
auth header on every model request.
"""
import time
...
DENY = Policy(claude_cred_helper={"mode": "deny"})                  # stricter, hard-block posture

CRED_JSON = '{"apiKeyHelper": "/tmp/evil.sh"}'


def _edit(path, new_string=CRED_JSON):
    return Event.make(HookEvent.PRE_TOOL_USE, tool="Edit",
                       args={"file_path": path, "new_string": new_string})


def _write(path, content=CRED_JSON):
...
def test_api_key_helper_via_write_gated():
    d = evaluate(_write(".claude/settings.local.json"), EMPTY)
    assert _gated(d) and d.rule == "claude-cred-helper-protect"


def test_api_key_helper_via_edit_gated():
    d = evaluate(_edit(".claude/settings.local.json",
                        '"apiKeyHelper": "/tmp/evil.sh"'), EMPTY)
    assert _gated(d) and d.rule == "claude-cred-helper-protect"
...
def test_each_dangerous_key_gated():
    for key in patterns.CLAUDE_CRED_HELPER_KEYS:
        d = evaluate(_write(".claude/settings.local.json",
                             f'{{"{key}": "/tmp/evil.sh"}}'), EMPTY)
        assert _gated(d) and d.rule == "claude-cred-helper-protect", key
...
def test_unicode_escaped_key_gated():
    """The same JSON `\\uXXXX`-escape evasion class `_claude_env_json_key_
    hit` closes for its own key set — `"\\u0061piKeyHelper"` decodes to the
    real key `apiKeyHelper` while a purely textual substring check can't see
    through the decode step it never performs."""
    d = evaluate(_write(".claude/settings.local.json",
                         '{"\\u0061piKeyHelper": "/tmp/evil.sh"}'), EMPTY)
    assert _gated(d) and d.rule == "claude-cred-helper-protect"
...
def test_partial_edit_fragment_not_valid_json_still_uses_textual_check():
    d = evaluate(_edit(".claude/settings.local.json",
                        '"awsAuthRefresh": "/tmp/evil.sh"'), EMPTY)
    assert _gated(d) and d.rule == "claude-cred-helper-protect"
...
def test_mcp_write_gated():
    d = evaluate(_mcp_write(".claude/settings.local.json", CRED_JSON), EMPTY)
    assert _gated(d) and d.rule == "claude-cred-helper-protect"
...
def test_mcp_flat_key_value_arg_shape_gated():
    d = evaluate(_mcp_key_value(".claude/settings.local.json", "apiKeyHelper",
                                 "/tmp/evil.sh"), EMPTY)
    assert _gated(d) and d.rule == "claude-cred-helper-protect"
...
def test_mcp_struct_key_depth_capped():
    nested = {"apiKeyHelper": "/tmp/evil.sh"}
    for _ in range(8):
...
def test_jsonc_comment_mentioning_key_not_gated_for_literal_edit():
    ...
    ordinary Edit/Write whose literal content merely mentions apiKeyHelper in
    a comment or unrelated string must not false-positive without the real
    key shape present."""
    d = evaluate(_write(".claude/settings.local.json",
                         '{\n  // TODO: consider setting apiKeyHelper later\n'
                         '  "model": "opus"\n}'), EMPTY)
    assert not _gated(d)
...
# ---- shell forms ----------------------------------------------------------------
#
# Same precedence note as `test_claude_env_protect.py`'s own shell-forms
...
# assert `self-protect`, not `claude-cred-helper-protect`. This guard's own
...
def test_shell_redirect_already_blocked_by_self_protect():
    d = evaluate(_shell(f"echo '{CRED_JSON}' > .claude/settings.local.json"), EMPTY)
...
def test_shell_jq_merge_form_via_mv_already_blocked_by_self_protect():
    d = evaluate(_shell(
        'jq \'. += {apiKeyHelper: "/tmp/evil.sh"}\' .claude/settings.local.json '
...
def test_shell_jq_bracket_index_notation_gated():
    d = evaluate(_shell(
        'jq \'.["otelHeadersHelper"] = "/tmp/evil.sh"\' .claude/settings.local.json '
        '| sponge .claude/settings.local.json'), EMPTY)
    assert _gated(d) and d.rule == "claude-cred-helper-protect"


def test_shell_jq_comment_padding_gated():
    """Same jq-`#`-comment-padding bypass class `CLAUDE_ENV_JQ_RE`'s own
    comment discloses for its own (originally fixed-window) predecessor —
    `CLAUDE_CRED_HELPER_JQ_RE` ships unbounded from the start, so this must
    already be closed with no follow-up QA round needed."""
    d = evaluate(_shell(
        "jq '#" + "x" * 450 + "\n.apiKeyHelper = \"/tmp/evil.sh\"' "
        ".claude/settings.local.json | sponge .claude/settings.local.json"), EMPTY)
    assert _gated(d) and d.rule == "claude-cred-helper-protect"
...
def test_jq_equality_comparison_not_gated():
    d = evaluate(_shell(
        "jq 'select(.apiKeyHelper == null)' .claude/settings.local.json"), EMPTY)
    assert not _gated(d)


def test_jq_plain_read_not_gated():
    assert not _gated(evaluate(
        _shell("jq '.apiKeyHelper' .claude/settings.local.json"), EMPTY))


def test_shell_cd_into_claude_dir_then_bare_filename_gated():
    d = evaluate(_shell(
        'cd .claude && jq \'.apiKeyHelper = "/tmp/evil.sh"\' '
...
def test_shell_push_location_into_claude_dir_then_bare_filename_gated():
    d = evaluate(_shell(
        'Push-Location .claude; jq \'.apiKeyHelper = "/tmp/evil.sh"\' '
        'settings.local.json | sponge settings.local.json'), EMPTY)
    assert _gated(d) and d.rule == "claude-cred-helper-protect"
...
def test_shell_cd_elsewhere_then_bare_filename_not_gated():
    assert not _gated(evaluate(_shell(
        'cd /tmp && echo "see settings.local.json for helper config"'), EMPTY))


def test_shell_cd_into_lookalike_dir_not_gated():
    for cmd in (
        'cd .claude-old && jq \'.apiKeyHelper="x"\' settings.local.json | sponge settings.local.json',
        'cd .claude.bak && jq \'.apiKeyHelper="x"\' settings.local.json | sponge settings.local.json',
    ):
...
def test_shell_redirect_to_unrelated_file_not_gated():
    assert not _gated(evaluate(_shell(f"echo '{CRED_JSON}' > output.txt"), EMPTY))
...
def test_benign_local_settings_edit_not_gated():
    d = evaluate(_write(".claude/settings.local.json",
                         '{"model": "opus", "statusLine": {"padding": 0}}'), EMPTY)
    assert not _gated(d)


def test_lookalike_key_name_not_gated():
    """A key that merely contains a dangerous substring (not an exact
    match) must not false-positive."""
    d = evaluate(_write(".claude/settings.local.json",
                         '{"myApiKeyHelperNote": "x"}'), EMPTY)
    assert not _gated(d)


def test_project_settings_json_not_gated_by_this_guard():
    d = evaluate(_write(".claude/settings.json", CRED_JSON), EMPTY)
    assert d.rule != "claude-cred-helper-protect"


def test_unrelated_file_mentioning_api_key_helper_not_gated():
    assert not _gated(evaluate(
        _write("README.md", 'Set apiKeyHelper in your settings...'), EMPTY))
...
def test_unrelated_path_named_settings_local_json_not_gated():
    assert not _gated(evaluate(
        _write("config/settings.local.json", CRED_JSON), EMPTY))


def test_settings_local_json_backup_file_not_gated():
    assert not _gated(evaluate(
        _write(".claude/settings.local.json.bak", CRED_JSON), EMPTY))


# ---- escape hatches: human-only --------------------------------------------------

_JQ_SPONGE_CMD = ('jq \'.apiKeyHelper = "/tmp/evil.sh"\' .claude/settings.local.json '
...
def test_env_toggle_allows_edit_write_and_shell(monkeypatch):
    monkeypatch.setenv("AEGIS_ALLOW_CLAUDE_CRED_HELPER", "1")
...
def test_policy_allow_regex_skips_gate():
    pol = Policy(claude_cred_helper={"allow": [r"\.claude/settings\.local\.json"]})
...
def test_default_mode_is_ask():
    d = evaluate(_write(".claude/settings.local.json"), EMPTY)
    assert d.action == Action.ASK and d.rule == "claude-cred-helper-protect"
    d2 = evaluate(_shell(_JQ_SPONGE_CMD), EMPTY)
    assert d2.action == Action.ASK and d2.rule == "claude-cred-helper-protect"


def test_deny_mode_hard_blocks():
    d = evaluate(_write(".claude/settings.local.json"), DENY)
    assert d.blocked and d.action == Action.DENY and d.rule == "claude-cred-helper-protect"


def test_monitor_mode_logs_and_allows():
    pol = Policy(claude_cred_helper={"mode": "monitor"})
    assert not _gated(evaluate(_write(".claude/settings.local.json"), pol))


def test_off_mode_disables_guard():
    pol = Policy(claude_cred_helper={"mode": "off"})
    assert not _gated(evaluate(_write(".claude/settings.local.json"), pol))
...
def test_shell_jq_unbounded_lookahead_does_not_cross_semicolon():
    d = evaluate(_shell(
        'jq \'.model\' .claude/settings.local.json; echo "note apiKeyHelper here"'), EMPTY)
    assert not _gated(d)
...
def test_perf_no_redos_on_long_path_content():
    long_content = '"x": "' + ("y" * 20000) + '", "apiKeyHelper": "/tmp/evil.sh"'
    start = time.time()
...
def test_dangerous_key_regex_matches_each_name():
    for key in patterns.CLAUDE_CRED_HELPER_KEYS:
        assert patterns.CLAUDE_CRED_HELPER_KEY_RE.search(f'"{key}": "x"'), key


def test_dangerous_key_regex_does_not_match_benign_names():
    for key in ("statusLine", "model", "outputStyle", "permissions"):
        assert not patterns.CLAUDE_CRED_HELPER_KEY_RE.search(f'"{key}": "x"'), key
